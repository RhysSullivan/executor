...
export const deriveWorkspaceMembershipsForPrincipal = (input: {
  ...
  const hasActiveOrganizationMembership = input.organizationMemberships.some(
    (membership) =>
      membership.accountId === input.principalAccountId
      && membership.organizationId === workspace.organizationId
      && membership.status === "active",
  );

  if (
    workspace.createdByAccountId === input.principalAccountId
  ...
  for (const membership of input.organizationMemberships) {
    ...
    if (membership.organizationId !== workspace.organizationId) {
      continue;
    }
