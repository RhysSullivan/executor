...
const toEffectElicitationHandler = (handler: ElicitationHandler) =>
  (ctx: ElicitationContext) =>
    Effect.tryPromise({ try: () => handler(ctx), catch: (e) => e as Error }).pipe(
      Effect.map((r) => new ElicitationResponseClass({ action: r.action, content: r.content })),
      Effect.catchAll((e) => Effect.die(e)),
    );

const toEffectInvokeOptions = (options: InvokeOptions): EffectInvokeOptions => ({
  onElicitation: options.onElicitation === "accept-all"
    ? ("accept-all" as const)
...
const effectToPromiseInvokeOptions = (options?: EffectInvokeOptions): InvokeOptions => {
  ...
  return { onElicitation: async (ctx) => { const r = await run(handler(ctx)); return { action: r.action, content: r.content ?? undefined }; } };
};

const toEffectInvoker = (invoker: ToolInvoker): EffectToolInvoker => ({
  invoke: (toolId, args, options) => fromPromise(() => invoker.invoke(toolId, args, effectToPromiseInvokeOptions(options))) as Effect.Effect<ToolInvocationResult, any>,
  resolveAnnotations: invoker.resolveAnnotations ? (toolId) => fromPromise(() => invoker.resolveAnnotations!(toolId)) as Effect.Effect<ToolAnnotations | undefined> : undefined,
});

const toEffectRuntimeHandler = (handler: RuntimeToolHandler): EffectRuntimeToolHandler => ({
  invoke: (args, options) => fromPromise(() => handler.invoke(args, effectToPromiseInvokeOptions(options))) as Effect.Effect<ToolInvocationResult, any>,
  resolveAnnotations: handler.resolveAnnotations ? () => fromPromise(() => handler.resolveAnnotations!()) as Effect.Effect<ToolAnnotations | undefined> : undefined,
});
...
const toEffectSecretProvider = (provider: SecretProvider): EffectSecretProvider => ({
  key: provider.key, writable: provider.writable,
...
type PluginExtensions<TPlugins extends readonly AnyPlugin[]> = {
  readonly [P in TPlugins[number] as P["key"]]: P extends Plugin<string, infer TExt>
    ? TExt : P extends ExecutorPlugin<string, infer TExt> ? Promisified<TExt> : never;
};
...
function promisifyObject<T extends object>(obj: T): Promisified<T> {
  return new Proxy(obj, {
    get(target, prop, receiver) {
      ...
      if (typeof value === "function") return (...args: unknown[]) => { const result = value.apply(target, args); if (Effect.isEffect(result)) return run(result as Effect.Effect<unknown, unknown>); return result; };
...
export const createExecutor = async <const TPlugins extends readonly AnyPlugin[] = []>(
  ...
  const effectPlugins = (config.plugins ?? []).map((p) =>
    isPromisePlugin(p as any) ? toEffectPlugin(p as Plugin<string, object>) : (p as unknown as ExecutorPlugin<string, object>),
  );
  const effectConfig: EffectExecutorConfig<ExecutorPlugin<string, object>[]> = {
    scope: { id: ScopeId.make(config.scope?.id ?? "default"), name: config.scope?.name ?? "default", createdAt: new Date() },
    tools: makeInMemoryToolRegistry(), sources: makeInMemorySourceRegistry(),
    secrets: makeInMemorySecretStore(), policies: makeInMemoryPolicyEngine(),
  ...
  const base: Record<string, unknown> = {
    ...
    tools: {
      ...
      invoke: (toolId: string, args: unknown, options: InvokeOptions) => run(executor.tools.invoke(toolId, args, toEffectInvokeOptions(options))),
    },
    sources: {
      list: () => run(executor.sources.list()), remove: (sourceId: string) => run(executor.sources.remove(sourceId)),
      refresh: (sourceId: string) => run(executor.sources.refresh(sourceId)), detect: (url: string) => run(executor.sources.detect(url)),
    },
    policies: {
    ...
    secrets: {
      ...
      set: (input: { readonly id: string; readonly name: string; readonly value: string; readonly provider?: string; readonly purpose?: string }) => run(executor.secrets.set(input as any)),
