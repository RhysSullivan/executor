// ---------------------------------------------------------------------------
// Structural SQLite storage core
//
// Driver-agnostic `makeSqliteStorage` used by both the `better-sqlite3`
// (Node) and `bun:sqlite` (Bun) variants. The database handle must
// expose the drizzle `run`/`all`/`get` interface for SQL templates; any
// drizzle driver for SQLite satisfies this.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Structural database interface — compatible with drizzle-orm's
// BetterSQLite3Database and BunSQLiteDatabase.
// ---------------------------------------------------------------------------

export interface SqliteRunner {
  readonly run: (query: SQL) => { readonly changes: number };
  readonly all: <T = Record<string, unknown>>(query: SQL) => readonly T[];
  readonly get: <T = Record<string, unknown>>(query: SQL) => T | undefined;
}
...
export const sqliteCapabilities = {
  supportsJSON: true,
  supportsDates: true,
...
export const makeSqliteStorage = (
  ...
  Effect.gen(function* () {
    ...
    yield* validateSchemaCapabilities("sqlite", sqliteCapabilities, schema);
...
const makeStorage = (db: SqliteRunner, schema: ExecutorDBSchema): ExecutorStorage => {
...
const migrate = (db: SqliteRunner, schema: ExecutorDBSchema) =>
  Effect.gen(function* () {
...
const findOne = <T>(db: SqliteRunner, schema: ExecutorDBSchema, args: FindOneArgs) =>
...
const run = (db: SqliteRunner, model: string, query: SQL) =>
  Effect.try({
    try: () => db.run(query),
    catch: (cause) =>
...
const all = <T>(db: SqliteRunner, model: string, query: SQL) =>
  Effect.try({
    try: () => db.all<T>(query) as T[],
    catch: (cause) =>
      new StorageQueryError({
        model,
        message: cause instanceof Error ? cause.message : "SQLite query failed",
...
const get = <T>(db: SqliteRunner, model: string, query: SQL) =>
  Effect.try({
    try: () => db.get<T>(query) as T | undefined,
    catch: (cause) =>
      new StorageQueryError({
        model,
        message: cause instanceof Error ? cause.message : "SQLite query failed",
