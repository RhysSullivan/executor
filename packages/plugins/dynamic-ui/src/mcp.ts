...
export const buildRenderUiDescription = (executeDescription: string): string => {
  ...
  return [
    ...
    "3. Recreate every read from the discovery step inside `App` with `useQuery(() => tools.<namespace>.<tool>(args))` so the UI stays live.",
    "4. Use `useMutation((input) => tools.<namespace>.<tool>(input))` for user-triggered writes or actions.",
    ...
    "- After discovering an API call with `execute`, put the same call in `useQuery` / `useMutation` inside the generated component.",
    "- Example discovery: call `execute` with `return await tools.axiom_mcp.querydataset({ ... })` to confirm columns, then call `render-ui` with `useQuery(() => tools.axiom_mcp.querydataset({ ... }))`.",
