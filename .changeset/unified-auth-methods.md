...
- Breaking (wire): the retired single-placement inputs (`headerName` on MCP, `in`/`name` on GraphQL) and raw canonical-placement inputs are rejected — author request-shaped. The `mcp.addServer` singular `auth` shorthand still works.
